...
void addParamsLayoutVars( const mndl::ConfigRef &config )
{
	...
	while ( TwWindowExists( windowId ) )
	{
		...
		for ( int i = 0; i < barCount; ++i )
		{
			...
			TwGetParam( bar, NULL, "size", TW_PARAM_INT32, 2, bi->mSize.ptr() );
			TwGetParam( bar, NULL, "position", TW_PARAM_INT32, 2, bi->mPos.ptr() );
...
void readParamsLayout()
{
	for ( const auto &bir : sBarInfos )
	{
		...
		TwSetParam( bar, NULL, "size", TW_PARAM_INT32, 2, bi->mSize.ptr() );
		TwSetParam( bar, NULL, "position", TW_PARAM_INT32, 2, bi->mPos.ptr() );
